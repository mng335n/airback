...
import com.airback.module.ecm.esb.DeleteResourcesEvent
import com.airback.module.ecm.esb.SaveContentEvent
...
class ResourceServiceImpl(private val contentJcrDao: ContentJcrDao,
    ...
    override fun moveResource(srcRes: Resource, destRes: Resource, userMove: String, sAccountId: Int?) {
        ...
        if (destRes is Content)
            throw airbackException("You can't move somethings to content path.That is impossible.")
        if (isDuplicateFileName(srcRes, destRes)) {
            throw airbackException("Please check duplicate file, before move")
        }
