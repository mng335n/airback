...
object PresenterResolver {
     ...
     @JvmStatic fun <P : IPresenter<*>> getPresenter(presenterClass: Class<P>): P {
        var presenterMap = airbackSession.getCurrentUIVariable(airbackSession.PRESENTER_VAL) as MutableMap<Class<*>, Any>?
        if (presenterMap == null) {
            presenterMap = mutableMapOf()
            airbackSession.putCurrentUIVariable(airbackSession.PRESENTER_VAL, presenterMap)
        }
