...
import com.airback.core.airbackThread;
...
public class StreamDownloadResourceSupportExtDrive implements StreamResource.StreamSource {
    ...
    public InputStream getStream() {
        ...
        Thread threadExport = new airbackThread(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    private void addFileToZip(String path, Content res, ZipOutputStream zip) throws Exception {
        ...
        InputStream contentStream = resourceService.getContentStream(res.getPath());
        if (path.length() == 0)
            path = res.getName();
        else
            path += "/" + res.getName();
    ...
    private void addFolderToZip(String path, Resource res, ZipOutputStream zip) throws Exception {
        List<Resource> lstResource = resourceService.getResources(res.getPath());
