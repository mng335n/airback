...
class ServerConfiguration(var storageSystem: String = STORAGE_FILE, var port: Int = 8089,
                          var address: String = "",
    ...
    fun getHomeDir(): File {
        if (_cacheHomeDir == null) {
            val userFolder = when (dataDir) {
                ...
                else -> throw airbackException("Not support data dir value $dataDir")
            }
            _cacheHomeDir = File("$userFolder/.airback")
            val userHomeDir = File(System.getProperty("user.dir") + "/.airback")
