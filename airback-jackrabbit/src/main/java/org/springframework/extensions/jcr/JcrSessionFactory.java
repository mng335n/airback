/**
 * This file is part of airback-jackrabbit.
 *
 * airback-jackrabbit is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * airback-jackrabbit is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with airback-jackrabbit.  If not, see <http://www.gnu.org/licenses/>.
 */
...
public class JcrSessionFactory implements InitializingBean, DisposableBean,
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(JcrSessionFactory.class);
	 ...
	 * subclass. By default, this method doesn't do anything.
	 * 
	 * @throws Exception
	 */
	 ...
	 * own subclass. By default, this method doesn't do anything.
	 * 
	 * @throws Exception
	 */
	...
	protected void registerNamespaces() throws Exception {
		...
		if (forceNamespacesRegistration) {
			...
			if (!keepNewNamespaces)
				overwrittenNamespaces = new HashMap<String, String>(
						namespaces.size());
			...
			for (Object key : namespaces.keySet()) {
				...
				if (position >= 0) {
					...
					if (!keepNewNamespaces) {
						// save old namespace
						overwrittenNamespaces.put(prefix,
		...
		for (Map.Entry<?, ?> entry : namespaces.entrySet()) {
			// Map.Entry<String, String> namespace = (Map.Entry<String, String>)
			// entry;
			...
			if (skipExistingNamespaces && position >= 0) {
				...
				LOG.debug("registering namespace [" + ns + "] under ["
						+ prefix + "]");
	 ...
	 * <p/>
	 * However, depending on the {@link #setKeepNewNamespaces(boolean)} setting,
	 * the old namespaces can be registered back once the application context is
	 * destroyed. False by default.
	 * 
	 * @param forceNamespacesRegistration
	 *            The forceNamespacesRegistration to set.
	 */
	public void setForceNamespacesRegistration(
			boolean forceNamespacesRegistration) {
	 ...
	 * <p/>
	 * This flag is required for JCR implementations which do not support
